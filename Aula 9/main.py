...
from estatisticas import estatisticas_tabuleiro, ordenar_posicoes_atacadas
...
def processar_ataque(coord, tamanho, tabuleiro, embarcacoes_do_jogo):
    ...
    if not acertou_alguma:
        ...
        if tabuleiro[linha_idx, col_idx] == "~":
            tabuleiro[linha_idx, col_idx] = "O" # Marca erro (água) no tabuleiro,all:true}],path:
...
def exibir_estatisticas(tabuleiro):
    ...
    print(f"Posições não atacadas: {stats['nao_atacadas']}")
    print(f"Porcentagem de acertos: {stats['porcentagem_acertos']:.2f}%")
    
    print("\nPosições Atacadas (Linha, Coluna):")
    if posicoes_ordenadas.size > 0:
        # Formata a saída para ser mais legível
        for r, c in posicoes_ordenadas:
            # A linha é r+1 (1-indexada) e a coluna é a letra (alfabeto[c])
            print(f"({r+1}, {alfabeto[c]})", end=" ")
        print()
    else:
        print("Nenhuma posição foi atacada ainda.")
    print("-----------------------------------")
...
def loop_principal_jogo(tamanho, tabuleiro, cabecalhoDasColunas, embarcacoes_do_jogo):
    ...
    while not todas_embarcacoes_afundadas(embarcacoes_do_jogo):
        ...
        while True:
            coord = input("Digite a coordenada de ataque (Ex: A1, B2), 'ESTATISTICAS' ou 'SAIR' para encerrar:\n").upper()
            
            if coord == 'SAIR':
                print("Encerrando o jogo a pedido do usuário.")
                return # Sai da função, encerrando o jogo
            
            if coord == 'ESTATISTICAS':
                exibir_estatisticas(tabuleiro)
                continue # Volta para o loop de coleta de coordenadas
