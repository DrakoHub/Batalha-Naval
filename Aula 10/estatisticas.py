...
def contar_afundamentos(tabuleiro: np.ndarray) -> int:    
    # Retorna o número total de afundamentos (posições marcadas com 'F').
    return np.sum(tabuleiro == SIMBOLO_AFUNDOU)
...
def estatisticas_tabuleiro(tabuleiro: np.ndarray) -> dict:
    ...
    acertos = contar_acertos(tabuleiro)+contar_afundamentos(tabuleiro)
...
def ordenar_posicoes_atacadas(tabuleiro: np.ndarray) -> np.ndarray:
    ...
    # Obter os índices (linhas e colunas) das posições True na máscara
